...
- **Play from wherever you scrubbed** — hitting Play resumes from the point last scrubbed to or clicked in the timeline/trim panel, instead of always restarting the sequence from the beginning.
...
- **Undo/redo** — Ctrl+Z / Ctrl+Y (or Ctrl+Shift+Z) step back and forward through up to 50 scene-stack changes, with matching toolbar buttons. A whole trim drag counts as a single undo step, not one per frame.
- **Overlay tracks B and C** — copy any clip from the main stack (track A) onto a second or third simultaneous track using the B/C buttons on its card. B/C clips play back and export at the same time as whatever's on A at that point in the timeline: video overlays on top, audio mixes in. Each track gets its own labeled bar under the main timeline.
- **Live audio-level meter** — a small bar next to the transport controls shows the current playback level in real time via a Web Audio `AnalyserNode`.
